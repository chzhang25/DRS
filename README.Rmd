...
`gammas.bs.11 - gammas.bs.19` (not shown) are estimates for approximating log baseline hazards $h_{01}(t)$, whereas `gammas.bs.21 - gammas.bs.29` (not shown) are estimates for approximating log baseline hazards $h_{02}(t)$.

`D1- D10` (not shown) are estimates for Choleski factors of the random effects covariance matrix ($\textbf{D}$).
